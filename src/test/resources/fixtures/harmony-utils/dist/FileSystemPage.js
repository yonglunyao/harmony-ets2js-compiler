...
export class FileSystemPage extends View {
  ...
  initialRender() {
    ...
    this.files = this.files.filter(/* arrow function */);
    }
    }
  });
            }
  ...
  listFiles() {
    FileManager.listFiles(this.currentPath).then(/* arrow function */).catch((error) => {
    Logger.error("列出文件失败", error);
  });
  }

  createDirectory() {
    FileManager.createDirectory(this.currentPath + '/new_folder').then(/* arrow function */).catch((error) => {
  ...
  currentPath__ = "/data/storage/el2/base/haps/entry/files";
