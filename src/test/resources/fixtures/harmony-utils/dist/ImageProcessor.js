...
export class ImageProcessor {
  ...
  const imagePacker = image.createImagePacker();
  const packingOpts = {format: "image/jpeg", quality: 100};
  const packedData = await imagePacker.packToData(pixelMap, packingOpts);
  const result = `data:image/jpeg;base64,${packedData}`;
  fs.closeSync(file);
  ...
  const pixelMap = await imageSource.createPixelMap();
  const imagePacker = image.createImagePacker();
  const packingOpts = {format: targetFormat, quality: 100};
  const packedData = await imagePacker.packToData(pixelMap, packingOpts);
  ...
  const pixelMap = await imageSource.createPixelMap();
  const imageInfo = await pixelMap.getImageInfo();
  const result = {width: imageInfo.size.width, height: imageInfo.size.height, format: "image/jpeg"};
  fs.closeSync(file);
  Logger.success("获取图片信息成功");
  return result;
} catch (error) {
  Logger.error("获取图片信息失败", error);
  const emptyResult = {width: 0, height: 0, format: ""};
  return emptyResult;
}
  }
