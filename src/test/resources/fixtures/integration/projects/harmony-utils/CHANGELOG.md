...
## 1.4.0（DevEco Studio 6.0.0 Release）
1. FileUtil：  
1、新增方法：getDownloadPath、getDownloadUri、saveToDownload。    
2、优化内部方法实现。    
2. ObjectUtil：    
1、新增方法：plainToClass、plainToClassArray、classToPlain、has。    
2、优化内部方法实现，如：shallowCopy、deepCopy、等。    
3. JSONUtil：    
1、优化内部方法实现，支持嵌套类。

## 1.3.9（DevEco Studio 6.0.0 Release）
1. 加解密方法入参优化。
2. FileUtil的hasDirPath()方法优化。
3. NetworkUtil代码优化。
4. PR:修复PermissionUtil在部分场景无法获取context的问题。（感谢汤于艺）。     

## 1.3.8（DevEco Studio 5.1.0 Release）
1. FileUtil的getFormatFileSize()方法优化。
2. JSONUtil的jsonToBean()方法优化。
3. ToastUtil代码优化。

## 1.3.7（DevEco Studio 5.1.0 Release）
1. DateUtil，Bug修复与代码优化。
2. RegexUtil，Bug修复。
3. WindowUtil，新增功能方法。
4. WantUtil，Bug修复。
5. NotificationUtil，新增方法 openNotificationSettings、requestEnableNotification。
