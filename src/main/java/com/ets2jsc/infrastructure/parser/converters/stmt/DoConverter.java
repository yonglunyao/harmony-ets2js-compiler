    ...
    protected String formatBody(AstNode stmt) {
        ...
        if (stmt instanceof Block block) {
            for (AstNode blockStmt : block.getStatements()) {
                String stmtCode = blockStmt.accept(new com.ets2jsc.infrastructure.generator.CodeGenerator());
                sb.append("  ").append(stmtCode);
            }
        } else {
            String stmtCode = stmt.accept(new com.ets2jsc.infrastructure.generator.CodeGenerator());
            sb.append("  ").append(stmtCode);
        }
        return sb.toString();
    }
